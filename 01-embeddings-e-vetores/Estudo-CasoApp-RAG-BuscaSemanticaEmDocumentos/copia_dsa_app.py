...
# Define o nome do modelo de embeddings a ser utilizado - Modelo otimizado para RAG que respeita o limite de 2GB de VRAM
...
CHROMA_COLLECTION_NAME = "itau_esg_2024"
...
def bbe_get_embedding_model():
    ...
    # Lotes menores (8) evitam o estouro de memória durante a vetorização
...
def bbe_get_vector_store(_uploaded_file, embedding_model):
...
st.set_page_config(page_title = "AI ESG Compliance", page_icon = "🌍", layout = "wide")

# Define o título principal da aplicação na interface
st.title("AI Financial Insights - Governança & ESG")

# Define o subtítulo destacando o estudo de caso e o uso de RAG
st.title("🤖 Auditor RAG para Compliance Climático")

# Exibe um texto explicativo sobre a tecnologia usada no app
st.markdown(f"Extração de alta precisão em relatórios regulatórios densos. Pipeline orquestrado com **Llama-3.3-70B (Groq)**, **intfloat/multilingual-e5-small Embeddings** e **ChromaDB**.")
...
embeddings = bbe_get_embedding_model()
...
with st.sidebar:
    ...
    uploaded_file = st.file_uploader(
        "Carregue seu relatório ESG (.pdf)", 
        type = "pdf",
    ...
    with st.sidebar.expander("🆘 Suporte / Fale conosco", expanded = False):
        ...
        st.write("Se tiver dúvidas envie mensagem para brnfa2011@gmail.com")
...
if uploaded_file:
    ...
    st.session_state.retriever = bbe_get_vector_store(uploaded_file, embeddings)
...
# Caso não haja upload, verifica se já existe um diretório persistido do Chroma
    ...
    if st.session_state.retriever is None: # Só carrega na primeira vez
        ...
        st.session_state.retriever = vectorstore.as_retriever(search_kwargs = {"k": 15})
...
if st.session_state.retriever:
    ...
    RAG_PROMPT_TEMPLATE = """
    Você é um Auditor de IA especializado em Compliance ESG e Risco Climático do setor bancário.
    Sua tarefa é responder a perguntas complexas sobre relatórios de sustentabilidade, emissões de carbono e governança corporativa, utilizando APENAS o contexto fornecido.
    Seja analítico, extremamente preciso com números (como toneladas de CO2e, valores financeiros e metas) e baseie-se exclusivamente nos dados dos trechos recuperados.
    Não deduza, não alucine dados e não utilize conhecimentos externos.
    Se a informação não estiver no contexto, diga estritamente: "A informação não foi encontrada nos documentos de Compliance analisados."
    ...
    bbe_rag_chain = (
        {"context": st.session_state.retriever | bbe_formata_docs, "question": RunnablePassthrough()}
        | rag_prompt
        | llm
        | StrOutputParser()
    )
    ...
    question = st.text_input("Faça sua pergunta ao Auditor IA. Ex: Como o banco avalia o risco climático na concessão de crédito? ou Qual é o preço interno de carbono?", disabled = False)
    ...
    if question:
        ...
        with st.spinner("Fazendo a busca semântica e gerando resposta..."):
            ...
            try:
                ...
                answer = bbe_rag_chain.invoke(question)
